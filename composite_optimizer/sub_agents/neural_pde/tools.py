...
"""Tools for neural PDE simulation using PINO models"""
...
def run_pino_simulation(parameters: Dict[str, Any]) -> dict:
    """
    Execute PINO simulation and return comprehensive results.
    ...
    try:
        # Convert to PINO format and set defaults if needed
...
def get_current_parameters() -> dict:
    """Extract current parameters from the latest simulation."""
    global _latest_pino_results
    ...
    try:
        ...
        current_params = {
            ...
            "Tool thickness Lt (cm)": process_params.get('tool_thickness', 0) * 100  # Convert back to cm
        }
...
def _ensure_pino_format(parameters: Dict[str, Any]) -> Dict[str, Any]:
    """Convert various parameter formats to PINO API format with defaults."""
    if "user_requirements_json" in parameters:
        ...
        return {
            ...
            "tool_thickness": float(user_req.get("Tool thickness Lt (cm)", 2.5)) / 100,  # Convert to meters
            "part_thickness": float(parameters.get("part_thickness", 3.0)) / 100  # Convert to meters
        }
    else:
        # Direct format - ensure all numeric and set defaults if missing
        defaults = {
        ...
        for key, default_value in defaults.items():
            result[key] = float(parameters.get(key, default_value))
