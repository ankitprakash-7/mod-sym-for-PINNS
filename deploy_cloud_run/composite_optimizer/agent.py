...
def extract_json_parameters(optimization_output: str) -> dict:
    ...
    try:
        ...
        required_params = [
            ...
            "Tool thickness Lt (m)"  # Expecting meters from optimization agent
        ]
        ...
        # FIXED: Validate parameter ranges using CORRECT parameter names
        constraints = {
            ...
            "Tool thickness Lt (m)": [0.02, 0.04]  # ✅ FIXED: Correct parameter name and meter range
        }
        
        violations = []
        for param, (min_val, max_val) in constraints.items():
            if param in user_params:  # Only validate parameters that exist
                try:
...
def ssssetract_json_parameters(optimization_output: str) -> dict:
    """
    Extract JSON parameters from optimization agent output for neural PDE simulation.
