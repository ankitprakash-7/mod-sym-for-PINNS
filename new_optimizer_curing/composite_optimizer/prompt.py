...
3. **MANDATORY OUTPUT PRESENTATION**: Present the COMPLETE optimization output including:
   - **JSON Parameter Block**: The structured parameters for neural PDE simulation
   - **📊 PERFORMANCE ANALYSIS vs OBJECTIVES**: Current results with exact gaps
   - **💡 PARAMETER ADJUSTMENTS**: Complete parameter table with old vs new values and reasoning
   - **Scientific Reasoning for Key Changes**: Brief explanations with literature citations
   - **Parameter Verification**: Constraint compliance confirmation
   - **🎯 READY FOR USER APPROVAL**: Summary with approval request
4. **Transition & Error Handling**:
   ...
   * **On Missing JSON**: If optimization output lacks the JSON parameter block at the beginning, inform user and ask if they want to retry optimization.
...
1. **User Communication**: Inform the user: "Approval received. I am now extracting the optimized parameters and running the simulation."
2. **Action**: 
...
3. **Parameter Extraction Process**: 
   - The optimization agent puts the JSON parameter block at the start of its output
   - Extract and validate the "user_requirements_json" structure
   - Ensure all 9 parameters are present and within constraint ranges
   - If extraction/validation fails, inform user and return to State 8
4. **MANDATORY OUTPUT PRESENTATION**: Present the COMPLETE simulation results exactly as specified in State 4.
5. **Transition & Error Handling**:
   ...
   * **On Parameter Extraction Failure**: Inform user: "Failed to extract valid parameters from optimization output. The JSON parameter block may be missing or malformed." Return to **State 8** for clarification.
   * **On Simulation Failure**: Inform the user the simulation failed with the optimized parameters. Return to **State 8** and ask if they want to adjust the parameters.
