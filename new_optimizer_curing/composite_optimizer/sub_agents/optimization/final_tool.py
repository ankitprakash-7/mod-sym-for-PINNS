...
def sssssssssssssssssssss() -> dict:
    """
    Hardcoded function for testing - returns simplified autoclave processing knowledge.
    """
    
    hardcoded_content = """
AUTOCLAVE PROCESSING GUIDELINES FOR COMPOSITE MATERIALS

SECTION 3.2 - HEATING RATES
Recommended heating rates for composite cure cycles:
- Thick sections (>2cm): 1.5-2.5°C/min to prevent thermal gradients
- Standard sections (1-2cm): 2.0-3.0°C/min for efficient processing
- Thin sections (<1cm): Up to 5.0°C/min acceptable

SECTION 4.1 - TEMPERATURE CONTROL
Hold temperatures should be selected based on:
- Gelation temperature: Typically 100-120°C for most prepreg systems
- Cure temperature: 175-185°C for standard epoxy systems
- Temperature uniformity: ±2°C across part thickness

SECTION 5.3 - HEAT TRANSFER COEFFICIENTS
Typical autoclave heat transfer coefficients:
- Top surface (direct air contact): 80-120 W/m²K
- Bottom surface (tooling interface): 40-80 W/m²K
- Tooling material affects bottom surface heat transfer significantly

SECTION 6.2 - THERMAL LAG MANAGEMENT
Thermal lag across thickness:
- Target: <15°C for quality cure
- Causes: High heating rates, thick sections, low thermal conductivity
- Solutions: Reduce heating rates, optimize heat transfer coefficients

SECTION 7.1 - EXOTHERM CONTROL
Exothermic reaction management:
- Target spike: <5°C above hold temperature
- Causes: Fast kinetics, thick sections, poor heat removal
- Solutions: Lower cure temperatures, improve heat transfer, extend ramp times

SECTION 8.3 - DEGREE OF CURE OPTIMIZATION
Cure completion requirements:
- Minimum DOC: 70% across thickness for structural applications
- DOC uniformity: <5% variation across thickness
- Time-temperature relationship critical for complete cure

TOOLING CONSIDERATIONS
- Aluminum tooling: High thermal conductivity, rapid heat transfer
- Steel tooling: Lower thermal conductivity, more gradual heating
- Tool thickness: 2-4cm typical for structural stability and thermal mass
"""

    return {
        "status": "success",
        "content": hardcoded_content,
        "message": "Successfully provided hardcoded autoclave processing knowledge",
        "url": "hardcoded_for_testing",
        "text_length": len(hardcoded_content),
        "page_count": 1,
        "extraction_method": "Hardcoded test content"
    }
...
def verifier(user_json: dict) -> dict:
    ...
    Args:
        user_json: Dictionary containing parameters to validate
