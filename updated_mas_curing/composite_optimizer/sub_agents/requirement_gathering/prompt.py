...
• Tool thickness Lt: [2.0, 5.0] cm
• Part thickness Lp: [2.5, 3.5] cm
```

**CONSTRAINT OPTIMIZATION STRATEGY:**
When material science suggests values outside these ranges:
1. **Use boundary values** (e.g., if science suggests 1.2°C/min, use 1.5°C/min minimum)
...
- BUT: Cannot go below 1.5°C/min, so if science suggests 1.0°C/min, use 1.5°C/min and compensate with extended hold times
- Extend hold times to ensure complete cure penetration (Kamal-Sourour kinetics)
- Adjust heat transfer coefficients based on autoclave capabilities WITHIN [70-120] and [50-100] ranges
...
**STRUCTURED PARAMETERS FOR SIMULATION:**
```json
{
  "user_requirements_json": {
    "Heating rate r1 (°C/min)": [actual_numeric_value],
    "Heating rate r2 (°C/min)": [actual_numeric_value],
    "Hold Temperature ht1 (°C)": [actual_numeric_value],
    "Hold Temperature ht2 (°C)": [actual_numeric_value],
    "Hold duration hd1 (min)": [actual_numeric_value],
    "Hold duration hd2 (min)": [actual_numeric_value],
    "Heat transfer coefficient top htop p (W/m2K)": [actual_numeric_value],
    "Heat transfer coefficient bottom hbot p (W/m2K)": [actual_numeric_value],
    "Tool thickness Lt (cm)": [actual_numeric_value],
    "Part thickness Lp (cm)": [actual_numeric_value]
  },
  "part_thickness": [actual_numeric_value],
  "material_type": "[material_name]",
  "tooling_material": "[tooling_material]",
  "tooling_thickness": [actual_numeric_value]
}
```
...
• Part thickness: [user_value] cm (Range: [2.5-3.5], Status: [✅ VALID/❌ ADJUSTED])
• Tool thickness: [user_value] cm (Range: [2.0-5.0], Status: [✅ VALID/❌ ADJUSTED])
...
- Heat transfer coefficient optimization for autoclave processing within [70-120] and [50-100] ranges
- Temperature ramp rate selection for quality cure within [1.5-3.0]°C/min bounds

**Composite Processing Knowledge:**
- Material-specific cure temperature requirements within [170-185]°C constraint
- Hold time optimization for complete cure within [50-65] and [105-120] min ranges
...
- Multi-parameter compensation when individual limits reached
...
- ALWAYS ensure all parameters fall within inference service compatible ranges
...
- ALWAYS use exact parameter names and units for system consistency
...
Remember: Generate parameters with scientific justification WITHIN mandatory inference service constraints and always document constraint impacts on optimization potential!
"""
